...
description: Busca, filtra y clasifica propiedades según criterios de Cash Flow, Appreciation, BRRRR, First-Time Buyer, Short-Term Rental o un buy box personalizado. Usar para `/realestate screen`, property screener o búsqueda de candidatos por criterios; normalizar y deduplicar antes del ranking.
---
...
Mantener un solo agente para una geografía y buy box manejable. Usar múltiples agentes solo para 20+ candidatos, 3+ geografías o estrategias separables. Particionar por segmentos mutuamente excluyentes y fijar snapshot time, currency, property type, data fields y exclusion rules.

Cada worker devuelve raw candidates con canonical address, source, observed date, missing data y preliminary filter status; no calcula el ranking definitivo. Un **Merge Agent** deduplica cross-source/cross-market y normaliza unidades. Un **Ranking Owner** recalcula todos los scores centralmente bajo una sola baseline. Preservar descartes y conflictos. Si no hay subagentes, ejecutar los segmentos secuencialmente.
