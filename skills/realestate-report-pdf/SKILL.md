...
description: Compila análisis PROPERTY-*.md existentes en un PDF inmobiliario profesional con tablas, scores, proyecciones, fuentes y limitaciones. Usar para `/realestate report-pdf` o cuando el usuario solicite un informe PDF client-ready a partir de análisis ya producidos.
---
...
Use the installed PDF/document workflow and load the workspace's bundled dependencies when generation or rendering is required. Do not assume scripts from another agent ecosystem exist. Preserve source analysis files and create the report only in the user-authorized output location.
...
Generate the PDF through the available PDF tooling, render it to page images, inspect every page and iterate until layout is correct. The report must contain the following sections:
